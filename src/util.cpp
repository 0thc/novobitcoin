...
const char *const BITCOIN_CONF_FILENAME = "novo.conf";
const char *const BITCOIN_PID_FILENAME = "novo.pid";
...
std::vector<std::string> ArgsManager::GetNonSensitiveParameters()
{
    ...
    // when starting novobitcoind (e.g. bitcoin -name) or they will not be added to mapMultiArgs.
...
// Windows < Vista: C:\Documents and Settings\Username\Application Data\NovoBitcoin
// Windows >= Vista: C:\Users\Username\AppData\Roaming\NovoBitcoin
// Mac: ~/Library/Application Support/NovoBitcoin
// Unix: ~/.novo-bitcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "NovoBitcoin";
#else
    fs::path pathRet;
...
#ifdef __APPLE__
    // Mac
    return pathRet / "Library/Application Support/NovoBitcoin";
#else
    // Unix
    return pathRet / ".novo-bitcoin";
#endif
#endif
}
