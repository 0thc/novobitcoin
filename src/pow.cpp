...
uint32_t GetNextASERTWorkRequired(const CBlockIndex *pindexPrev,
    ...
    assert(pindexPrev->GetHeight() >= params.asertAnchorParams.nHeight);
    ...
    const int nHeightDiff = pindexPrev->GetHeight() - params.asertAnchorParams.nHeight;
    ...
    if (params.SteadyASERTHeight > 0 && pindexPrev->GetHeight() >= params.SteadyASERTHeight)
...
uint32_t GetNextWorkRequired(const CBlockIndex *pindexPrev,
    ...
    if (IsASERTEnabled(config, pindexPrev->GetHeight())) {
