...
bool ReadKeyValue(CWallet *pwallet, CDataStream &ssKey, CDataStream &ssValue,
    ...
    try {
        ...
        if (strType == "name") {
            ...
            // Assume sig-op count as limit. It's unlikely that user stores invalid txs (those with
            // too high sigop count) in his wallet and would try to use them
            uint64_t maxTxSizeConsensus = config.GetMaxTxSize(true);
            bool isValid = wtx.IsCoinBase()
                               ? CheckCoinbase(wtx, state, maxTxSizeConsensus)
                               : CheckRegularTransaction(wtx, state, maxTxSizeConsensus);
