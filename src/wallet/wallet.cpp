    ...
    for (std::map<uint256, CWalletTx>::const_iterator it = mapWallet.begin();
        ...
        for (unsigned int i = 0; i < pcoin->tx->vout.size(); i++) {
            ...
            if (!(IsSpent(wtxid, i)) && mine != ISMINE_NO &&
                ...
                !IsP2SH(pcoin->tx->vout[i].scriptPubKey) && // we don't want to select p2sh utxos
...

    {
        ...
        while (true) {
            ...
            for (const auto &recipient : vecSend) {
                ...
                if (txout.IsDust(dustRelayFee, config.GetDustLimitFactor())) {
                    if (recipient.fSubtractFeeFromAmount &&
                        nFeeRet > Amount(0)) {
            ...
            if (nChange > Amount(0)) {
                ...
                if (nSubtractFeeFromAmount > 0 &&
                    newTxOut.IsDust(dustRelayFee, config.GetDustLimitFactor())) {
                    Amount nDust = newTxOut.GetDustThreshold(dustRelayFee, config.GetDustLimitFactor()) -
                                   newTxOut.nValue;
                    ...
                    for (unsigned int i = 0; i < vecSend.size(); i++) {
                        if (vecSend[i].fSubtractFeeFromAmount) {
                            ...
                            if (txNew.vout[i].IsDust(dustRelayFee, config.GetDustLimitFactor())) {
                                strFailReason =
                ...
                // the fee.
                if (newTxOut.IsDust(dustRelayFee, config.GetDustLimitFactor())) {
                    nChangePosInOut = -1;
                    nFeeRet += nChange;
        ...
        // Limit size.
        if (CTransaction(wtxNew).GetTotalSize() >= config.GetMaxTxSize(false)) {
...
const std::string &CWallet::GetAccountName(const CScript &scriptPubKey) const {
    ...
    if (CWallet::ExtractDestination(scriptPubKey, address) &&
        !scriptPubKey.IsKnownOpReturn()) { // we do not know how to spend coins containing OP_RETURN
...
void CWallet::GetScriptForMining(std::shared_ptr<CReserveScript> &script) {
    ...
    script = rKey;
    script->reserveScript = CScript() << OP_DUP << OP_HASH160 << ToByteVector(pubkey.GetID())
