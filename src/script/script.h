...
// Maximum number of bytes pushable to the stack -- replaced with DEFAULT_STACK_MEMORY_USAGE
...
public:
    ...
    novo::instruction_iterator begin_instructions() const;
    novo::instruction_iterator end_instructions() const;
     ...
     *  ... OP_N CHECKMULTISIG ...
     *
     * All sigops are counted accuratelly no matter how the flag is set.
     * More than 16 pub keys are supported, but the size of the number representing
     ...
     */
    uint64_t GetSigOpCount(bool& sigOpCountError) const;
...
bool IsP2SH(novo::span<const uint8_t>);
bool IsDustReturnScript (novo::span<const uint8_t> script);


size_t CountOp(novo::span<const uint8_t>, opcodetype);
