...
inline bool IsValidMaxOpsPerScript(uint64_t nOpCount,
                                   ...
                                   bool consensus)
{
    return (nOpCount <= config.GetMaxOpsPerScript(consensus));
}
    ...
    ScriptError* serror)
{
    ...
    const uint64_t maxScriptNumLength = config.GetMaxScriptNumLength(consensus);

    if(script.size() > config.GetMaxScriptSize(consensus))
    {
    ...
    // if OP_RETURN is found in executed branches,
    // we still have to check if the rest of the script is valid
    bool nonTopLevelReturn = false;
    ...
    try {
        while (pc < pend) {
            ...
            bool fExec = !count(vfExec.begin(), vfExec.end(), false) && (!nonTopLevelReturn || opcode == OP_RETURN);
            ...
            if ((opcode > OP_16) && !IsValidMaxOpsPerScript(++nOpCount, config, consensus)) {
                return set_error(serror, SCRIPT_ERR_OP_COUNT);
            }

            // Some opcodes are disabled.
            if (IsOpcodeDisabled(opcode) && fExec) {
            ...
            if (fExec && 0 <= opcode && opcode <= OP_PUSHDATA4) {
                ...
                switch (opcode) {
                    ...
                    case OP_ELSE: {
                        // Only one ELSE is allowed in IF.
                    ...
                    case OP_RETURN: {
                        ...
                        // op_return encountered inside if statement --> check for invalid grammar
                        nonTopLevelReturn = true;
                    } break;
                    ...
                    case OP_DEPTH: {
                        // -- stacksize
                        const CScriptNum bn(novo::bint{stack.size()});
                        stack.push_back(bn.getvch());
                    } break;
                    ...
                    case OP_SIZE: {
                        ...
                        CScriptNum bn(novo::bint{stack.stacktop(-1).size()});
                        stack.push_back(bn.getvch());
                    } break;
                    ...
                    case OP_LSHIFT:
                    {
                        ...
                        else
                        {
                            do
                            {
                                ...
                                if(token.IsCanceled())
                                    return {};
                                n -= CScriptNum{novo::bint{INT32_MAX}};
                            } while(n > 0);
                        }
                    ...
                    case OP_CHECKMULTISIGVERIFY: {
                        ...
                        if (nKeysCount > config.GetMaxPubKeysPerMultiSig(consensus)) {
                            return set_error(serror, SCRIPT_ERR_PUBKEY_COUNT);
                        }

                        nOpCount += nKeysCount;
                        if (!IsValidMaxOpsPerScript(nOpCount, config, consensus)) {
                            return set_error(serror, SCRIPT_ERR_OP_COUNT);
                        }
                        uint64_t ikey = ++i;
                    ...
                    default: {
                        if (IsInvalidBranchingOpcode(opcode) && !fExec)
                        {
                            break;
                        }
    ...
    catch(const novo::big_int_error&)
    {
    ...
    ScriptError* serror)
{
    ...
    LimitedStack stack(config.GetMaxStackMemoryUsage(consensus));
