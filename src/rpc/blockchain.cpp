                                           ...
                                           const std::string& rpcReqId)
{
    ...
    do
    {
        ...
        if (showTxDetails)
        {
            TxToJSON(transaction, uint256(), RPCSerializationFlags(), jWriter);
        }
        else
        {
...
UniValue gettxout(const Config &config, const JSONRPCRequest &request) {
    ...
    auto writeCoin =
        [&](const CoinWithScript& coin)
        {
            ...
            ScriptPubKeyToUniv(coin.GetTxOut().scriptPubKey, true, o);
    ...
    auto writeCoin =
        [&](const CoinWithScript& coin)
        {
            ...
            if(returnFieldsFlags & isStandardFlag)
            {
                ...
                jWriter.pushKV("isStandard", IsStandard(config, coin.GetTxOut().scriptPubKey, txOutType));
            }
...
UniValue getmempoolinfo(const Config &config, const JSONRPCRequest &request) {
    if (request.fHelp || request.params.size() != 0) {
        throw std::runtime_error(
            ...
            "  \"mempoolminfee\": xxxxx       (numeric) Minimum fee (in " + CURRENCY_UNIT + "/kB) for tx to be accepted\n"
            "}\n"
            "\nExamples:\n" +
...
Nothing (JSON null value) if successful and an error code otherwise.
    -1: Specified block cannot be marked as soft rejected.
        Response contains general error description while details are provided in novobitcoind log file.
    ...
    do
    {
        ...
        if (loop_inputs) {
            ...
            for (const CTxIn &in : tx->vin) {
                ...
                uint256 hashBlock;
                if (!GetTransaction(config, in.prevout.GetTxId(), tx_in, true, hashBlock)) {
