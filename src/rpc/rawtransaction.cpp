                       ...
                       std::function<void()> httpCallback)
{
    ...
    CTransactionRef tx;
    uint256 hashBlock;
    if (!GetTransaction(config, txid, tx, true, hashBlock))
    {
        throw JSONRPCError(
    ...
    if (!hashBlock.IsNull())
    {
        ...
        TxToJSON(*tx, hashBlock, RPCSerializationFlags(), jWriter, blockData);
    }
    else
    {
        TxToJSON(*tx, uint256(), RPCSerializationFlags(), jWriter);
    }
                                  ...
                                  bool verifyTxIds = true)
{
    ...
    if (pblockindex == nullptr)
    {
        ...
        if (!GetTransaction(config, *setTxIds.cbegin(), tx, false, foundBlockHash) ||
...
static UniValue decodescript(const Config &config,
    ...
    ScriptPubKeyToUniv(script, true, r);
    ...
    if (request.params.size() > 1 && !request.params[1].isNull()) {
        ...
        for (size_t idx = 0; idx < prevTxs.size(); idx++) {
...

            {
                ...
                view.AddCoin(out, CoinWithScript::MakeOwning(std::move(txout), coinHeight, false), true);
            }
