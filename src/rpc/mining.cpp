...
static UniValue GetNetworkHashPS(int lookup, int32_t height) {
    ...
    // If lookup is -1, then use 288 blocks before.
    if (lookup <= 0) {
        lookup = 288;
    }
...
static UniValue getnetworkhashps(const Config &config,
    ...
    if (request.fHelp || request.params.size() > 2) {
        throw std::runtime_error(
            ...
            "blocks, or -1 for 288 blocks before.\n"
...
// NOTE: Unlike wallet RPC, mining RPCs follow GBT (BIP
                      ...
                      bool processedInBatch = true)
{
...

    {
        ...
        int64_t defaultmaxBlockSize = config.GetChainParams().GetDefaultBlockSizeParams().maxGeneratedBlockSize;
