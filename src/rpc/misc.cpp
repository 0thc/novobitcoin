...
static UniValue getinfo(const Config &config, const JSONRPCRequest &request) {
    ...
    obj.push_back(Pair("maxstackmemoryusagepolicy",
                       config.GetMaxStackMemoryUsage(false)));
    obj.push_back(Pair("maxstackmemoryusageconsensus",
                       config.GetMaxStackMemoryUsage(true)));
    return obj;
}
...
static UniValue echo(const Config &config, const JSONRPCRequest &request) {
    if (request.fHelp) {
        throw std::runtime_error(
            ...
            "novobitcoin-cli. There is no server-side difference.");
    }
...
static UniValue getsettings(const Config &config, const JSONRPCRequest &request)
{
    ...
    if (request.fHelp || request.params.size() != 0)
    {
        throw std::runtime_error(
            ...
            "  \"blockmintxfee\": xxxxx,                 (numeric) Lowest fee rate (in " + CURRENCY_UNIT + "/kB) for "
            "transactions to be included in block creation\n"
            "  \"minrelaytxfee\": xxxxx,                 (numeric) Fees (in " + CURRENCY_UNIT + "/kB) smaller "
            "than this are considered zero fee for relaying, mining and transaction creation\n"
            "  \"dustrelayfee\": xxxxx,                  (numeric) Fee rate (in " + CURRENCY_UNIT + "/kB) used to define dust. A transaction "
    ...
    obj.push_back(Pair("maxtxsizepolicy", config.GetMaxTxSize(false)));
    obj.push_back(Pair("maxorphantxsize", config.GetMaxOrphanTxSize()));
    obj.push_back(Pair("datacarriersize", config.GetDataCarrierSize()));

    obj.push_back(Pair("maxscriptsizepolicy", config.GetMaxScriptSize(false)));
    obj.push_back(Pair("maxopsperscriptpolicy", config.GetMaxOpsPerScript(false)));
    obj.push_back(Pair("maxscriptnumlengthpolicy", config.GetMaxScriptNumLength(false)));
    obj.push_back(Pair("maxpubkeyspermultisigpolicy", config.GetMaxPubKeysPerMultiSig(false)));
    obj.push_back(Pair("maxtxsigopscountspolicy", config.GetMaxTxSigOpsCountPolicy()));
    obj.push_back(Pair("maxstackmemoryusagepolicy", config.GetMaxStackMemoryUsage(false)));
    obj.push_back(Pair("maxstackmemoryusageconsensus", config.GetMaxStackMemoryUsage(true)));
