...
bool static ScanHash(const CBlockHeader *pblock, uint32_t &nNonce, uint256 *phash)
{
    ...
    CSHA256 taghasher;
    taghasher.Write((uint8_t *)&taghash, CSHA256::OUTPUT_SIZE);
    taghasher.Write((uint8_t *)&taghash, CSHA256::OUTPUT_SIZE);
    ...
    while (true)
    {
        ...
        CSHA256(hasher).Write((unsigned char *)&nNonce, 4).Finalize((unsigned char *)phash);
        CSHA256(taghasher).Write((uint8_t *)phash, CSHA256::OUTPUT_SIZE).Finalize((unsigned char *)phash);

        // Return the nonce if the hash has at least some zero bits,
        // caller will check if it has enough to reach the target
        if (*std::next(phash->begin(), 31) == 0 &&
            *std::next(phash->begin(), 30) == 0)
            return true;
...
// Not ideal - a modified copy of the options from novobitcoin-cli
std::string HelpMessageCli()
{
     ...
     strUsage += HelpMessageOpt("-rpcwallet=<walletname>",
         _("Send RPC for non-default wallet on RPC server (argument is wallet "
           "filename in novobitcoind directory, required if novobitcoind runs with "
           "multiple wallets)"));
...
void Add_space_for_extra_nonce(vector<unsigned char>& coinbase_bytes, size_t offset_extra_nonce)
{
    ...
    tmp.push_back((unsigned char)0x04);
    ...
    tmp.insert(tmp.begin() + offset_extra_nonce + 1 + sizeof(extra_nonce_type),
                coinbase_bytes.begin() + offset_extra_nonce,
                coinbase_bytes.end());

    tmp[41] += 1 + sizeof(extra_nonce_type);
                               ...
                               RandomIntGenerator & random_int_func)
{
    ...
    while (!found)
    {
        // hashMerkleRoot:
        {
            ...
            memcpy(pbytes + offset_extra_nonce + 1, &nExtraNonce, sizeof(nExtraNonce));
            uint256 hash;
                                            ...
                                            RandomIntGenerator & random_int_func)
{
    ...
    if (bytes_used_for_height >= OP_1) {
        bytes_used_for_height = 0;
    }
    size_t offset_extra_nonce = 43 + bytes_used_for_height;

    if(coinbaseBytes.size() < offset_extra_nonce) // crude.
    {
        cerr << "Invalid coinbase transaction supplied, size" << coinbaseBytes.size() << endl;
        return false;
    }
    ...
    while ((GetTime() < start + searchDuration) && !found)
    {
        ...
        // and the block will be rejected.  So do not advance time (let it be advanced by novobitcoind every time we
        // request a new block).
        // header.nTime = (header.nTime < GetTime()) ? GetTime() : header.nTime;

        found = CpuMineBlockHasher(&header, coinbaseBytes, merkleproof, offset_extra_nonce, random_int_func);
    }
    ...
    printf("submit coinbase: %s\n", tmpstr.c_str());
    printf("submit version: %x parent: %s merkle: %s bits: %x time: %d nonce: %x\n",
           header.nVersion,
           header.hashPrevBlock.ToString().c_str(),
           header.hashMerkleRoot.ToString().c_str(),
           header.nBits, header.nTime, header.nNonce);
    ...
    while (0 != nblocks)
    {
        ...
        if (strPrint != "")
        {
            ...
            else
            {
                ...
                // The result is sent to novobitcoind above when the loop gets to it.
...
int main(int argc, char *argv[])
{
    ...
    try
    {
        std::string appname("novobitcoin-miner");
        std::string usage = "\n" + _("Usage:") + "\n" + "  " + appname + " [options] " + "\n";
