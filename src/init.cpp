...
#include "config/novobitcoin-config.h"
#endif
...
std::string HelpMessage(HelpMessageMode mode, const Config& config) {
    ...
    strUsage += HelpMessageOpt(
        ...
        strprintf(_("Set maximum stack memory usage used for script verification "
                  ...
                  DEFAULT_STACK_MEMORY_USAGE_POLICY/ONE_MEGABYTE));
    strUsage +=
        HelpMessageOpt("-maxopsperscriptpolicy=<n>",
            strprintf(_("Set maximum number of non-push operations "
                        "we're willing to relay/mine per script (default: unlimited, 0 = unlimited)")));
    strUsage += HelpMessageOpt(
        "-maxtxsigopscountspolicy=<n>",
        strprintf("Set maximum allowed number of signature operations we're willing to relay/mine in a single transaction (default: unlimited, 0 = unlimited)."));
    ...
    strUsage +=
        HelpMessageOpt("-maxtxsizepolicy=<n>",
            strprintf(_("Set maximum transaction size in bytes we relay and mine (default: %u MB, 0 = unlimited). "
                        "The value may be given in bytes or with unit (B, kB, MB, GB)."),
                DEFAULT_MAX_TX_SIZE_POLICY/ONE_MEGABYTE));
    ...
    strUsage += HelpMessageOpt(
        "-maxscriptsizepolicy",
        strprintf("Set maximum script size in bytes we're willing to relay/mine per script. "
            "(default: %u, 0 = unlimited). The value may be given in bytes or with unit (B, kB, MB, GB).",
            DEFAULT_MAX_SCRIPT_SIZE_POLICY));
    ...
    strUsage += HelpMessageOpt(
        ...
        strprintf(_("Set maximum block size in bytes we will mine. "
                    ...
                    "Mainnet: %d MB, "
                    "Testnet: %d MB."),
                    defaultChainParams->GetDefaultBlockSizeParams().maxGeneratedBlockSize / ONE_MEGABYTE,
                    testnetChainParams->GetDefaultBlockSizeParams().maxGeneratedBlockSize / ONE_MEGABYTE
                    ));
    strUsage += HelpMessageOpt(
        "-blockmintxfee=<amt>",
    ...
    strUsage += HelpMessageOpt(
        ...
        strprintf("Set maximum allowed number of public keys we're willing to relay/mine in a single CHECK_MULTISIG(VERIFY) operation (default: unlimited, 0 = unlimited)"));
...
std::string LicenseInfo() {
    const std::string URL_SOURCE_CODE =
        "<https://github.com/novobitcoin/novobitcoin>";
    const std::string URL_WEBSITE = "<https://novobitcoin.org>";
...
bool AppInitParameterInteraction(ConfigInit &config) {
    ...
    {
        int64_t maxTxSizePolicy = gArgs.GetArgAsBytes("-maxtxsizepolicy", DEFAULT_MAX_TX_SIZE_POLICY);
    ...
    if (std::string err; !config.SetMaxStackMemoryUsage(
        ...
        gArgs.GetArgAsBytes("-maxstackmemoryusagepolicy", DEFAULT_STACK_MEMORY_USAGE_POLICY),
        &err))
    {
        return InitError(err);
    }

    //Configure max script size
    if (gArgs.IsArgSet("-maxscriptsizepolicy")) {
        int64_t maxScriptSize = gArgs.GetArgAsBytes("-maxscriptsizepolicy", DEFAULT_MAX_SCRIPT_SIZE_POLICY);
    ...
    {
        const int64_t value = gArgs.GetArg("-maxtxsigopscountspolicy", DEFAULT_TX_SIGOPS_COUNT_POLICY);
    ...
    {
        const int64_t value = gArgs.GetArg("-maxpubkeyspermultisigpolicy", DEFAULT_PUBKEYS_PER_MULTISIG_POLICY);
