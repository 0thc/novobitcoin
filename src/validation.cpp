...
uint64_t GetSigOpCountForValidation(const CTransaction &tx, bool& sigOpCountError)
{
    sigOpCountError = false;
    uint64_t nSigOps = 0;
    for (const auto &txout : tx.vout)
    {
        nSigOps += txout.scriptPubKey.GetSigOpCount(sigOpCountError);
        if (sigOpCountError)
        {
            return nSigOps;
        }
...
uint64_t GetTransactionSigOpCount(const Config &config,
    ...
    sigOpCountError = false;
    uint64_t nSigOps = GetSigOpCountForValidation(tx, sigOpCountError);
    if (sigOpCountError) {
        return nSigOps;
    }
...
static bool CheckTransactionCommon(const CTransaction& tx,
    ...
    if (::GetSerializeSize(tx, SER_NETWORK, PROTOCOL_VERSION) < MIN_TX_SIZE_CONSENSUS) {
    ...
    // count sigops
    bool sigOpCountError;
    uint64_t nSigOpCount = GetSigOpCountForValidation(tx, sigOpCountError);
    if (sigOpCountError || nSigOpCount > MAX_TX_SIGOPS_COUNT) {
...
bool CheckCoinbase(const CTransaction& tx, CValidationState& state, uint64_t maxTxSizeConsensus)
{
    if (!tx.IsCoinBase()) {
    ...
    if (!CheckTransactionCommon(tx, state, maxTxSizeConsensus)) {
        // CheckTransactionCommon fill in the state.
        return false;
    }

    // see CMainParams()
    if (tx.vin[0].scriptSig.size() < 1 || tx.vin[0].scriptSig.size() > MAX_COINBASE_SCRIPTSIG_SIZE) {
...
bool CheckRegularTransaction(const CTransaction &tx, CValidationState &state, uint64_t maxTxSizeConsensus) {
    if (tx.IsCoinBase()) {
...
bool IsASERTEnabled(const Config &config, int32_t nHeight) {
    return nHeight >= config.GetChainParams().GetConsensus().asertAnchorParams.nHeight;
}
...
uint32_t GetScriptVerifyFlags(const Config &config) {
    // Check inputs based on the set of flags we activate.
    uint32_t scriptVerifyFlags = StandardScriptVerifyFlags();
...
void CommitTxToMempool(
    ...
    // non-final txns have their own mempool
    ...
    task::CTimedCancellationBudget& cancellationBudget)
{
    ...
    uint64_t maxTxSizeConsensus = config.GetMaxTxSize(true);
    // Coinbase is only valid in a block, not as a loose transaction.
    if (!CheckRegularTransaction(tx, state, maxTxSizeConsensus))
    {
    ...
    std::string reason;
    bool fStandard = IsStandardTx(config, tx, reason);
    if (fStandard) {
        state.SetStandardTx();
    }
    ...
    if(!fStandard)
    {
        if (fRequireStandard && reason != "scriptpubkey")
        {
    ...
    if (fUseLimits && (TxValidationPriority::low != pTxInputData->GetTxValidationPriority()))
    {
        auto res =
    ...
    const uint64_t nSigOpsCount{
                GetTransactionSigOpCount(config, tx, view, sigOpCountError)
    };
    // Check that the transaction doesn't have an excessive number of
    // sigops, making it impossible to mine. We consider this an invalid rather
    // than merely non-standard transaction.
    if (sigOpCountError || nSigOpsCount > config.GetMaxTxSigOpsCountPolicy()) {
        state.DoS(0, false, REJECT_NONSTANDARD,
    ...
    // We are getting flags as they would be.
    ...
    std::vector<CScriptCheck>* pvChecks)
{
    ...
    // ScriptExecutionCache does NOT contain per-input flags.
    ...
    else if (!res.value())
    {
        ...
        if (!consensus && SCRIPT_ERR_SCRIPTNUM_OVERFLOW == check.GetScriptError()) {
            return state.DoS(
...
DisconnectResult UndoCoinSpend(const CoinWithScript &undo, CCoinsViewCache &view,
    ...
    view.AddCoin(out, undo.MakeOwning(), !fClean);
...
uint32_t GetBlockScriptFlags(const Config& config, const CBlockIndex* pChainTip)
{
    ...
    uint32_t flags = SCRIPT_VERIFY_SIGPUSHONLY | SCRIPT_VERIFY_STRICTENC | SCRIPT_VERIFY_LOW_S | SCRIPT_VERIFY_NULLFAIL;
...
private:
        ...
        CBlockUndo& blockundo )
    {
        ...
        const uint64_t nMaxSigOpsCountConsensus = config.GetMaxBlockSigOpsConsensus(currentBlockSize);
        ...
        for (size_t i = 0; i < block.vtx.size(); i++) {
            ...
            // we count sigops when connecting blocks
            // GetTransactionSigOpCount counts 2 types of sigops:
            // * legacy (always)
            // * p2sh (when P2SH enabled)
            bool sigOpCountError;
            uint64_t txSigOpsCount = GetTransactionSigOpCount(config, tx, view, sigOpCountError);
            if (sigOpCountError || txSigOpsCount > MAX_TX_SIGOPS_COUNT) {
                return state.DoS(100, false, REJECT_INVALID, "bad-txn-sigops");
            }

            nSigOpsCount += txSigOpsCount;
            if (nSigOpsCount > nMaxSigOpsCountConsensus) {
...
bool CheckBlock(const Config &config, const CBlock &block,
    ...
    if (!CheckCoinbase(*block.vtx[0], state, maxTxSizeConsensus)) {
    ...
    uint64_t nSigOps = 0;
    auto nMaxSigOpsCountConsensus = config.GetMaxBlockSigOpsConsensus(currentBlockSize);
    ...
    size_t i = 0;
    while (true) {
        // we count sigops when verifying blocks
        // Count the sigops for the current transaction. If the total sigops
        // count is too high, the the block is invalid.
        bool sigOpCountError;
        nSigOps += GetSigOpCountForValidation(*tx, sigOpCountError);
        if (sigOpCountError || nSigOps > nMaxSigOpsCountConsensus) {
...
static bool RollforwardBlock(const CBlockIndex *pindex, CoinsDBSpan &inputs,
    ...
    while (!blockStreamReader->EndOfStream()) {
        ...
        AddCoins(inputs, *tx, pindex->GetHeight(), true);
    }
