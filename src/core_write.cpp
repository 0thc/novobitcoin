...
std::string EncodeHexBlk(const CBlockHeader& blk, const int serialFlags)
{
    CStringWriter stringWriter;
    EncodeHexBlk(blk, stringWriter, serialFlags);
    return stringWriter.MoveOutString();
}
...
void EncodeHexBlk(const CBlockHeader& blk, CTextWriter& writer, const int serialFlags)
{
    CHexWriter ssBlk(writer);
    ssBlk << blk;
}

void ScriptPubKeyToUniv(const CScript &scriptPubKey, bool fIncludeHex, UniValue &out) {
    txnouttype type;
              ...
              const std::optional<CBlockDetailsData>&  blockData)
{
    ...
    for (size_t i = 0; i < tx.vout.size(); i++)
    {
        ...
        ScriptPublicKeyToJSON(txout.scriptPubKey, true, entry);
        entry.writeEndObject();
