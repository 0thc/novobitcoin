 ...
 *
 * This is the developer documentation of Novo Bitcoin
 * (https://novobitcoin.org/). Novo Bitcoin is a client for the digital
 * currency called Novo Bitcoin, which enables
 * instant payments to anyone, anywhere in the world. Novo Bitcoin uses
    ...
    try {
        ...
        if (fCommandLine) {
            fprintf(stderr, "Error: There is no RPC client functionality in "
                            "novobitcoind anymore. Use the novobitcoin-cli utility "
                            "instead.\n");
            exit(EXIT_FAILURE);
        }
        // -server defaults to true for novobitcoind
...
int main(int argc, char *argv[]) {
    ...
    // Connect novobitcoind signal handlers
    noui_connect();
