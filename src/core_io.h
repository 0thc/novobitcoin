...
std::string EncodeHexBlk(const CBlockHeader &blk, const int serializeFlags = 0);
void EncodeHexTx(const CTransaction& tx, CTextWriter& writer, const int serializeFlags = 0);
void EncodeHexBlk(const CBlockHeader& blk, CTextWriter& writer, const int serializeFlags = 0);
