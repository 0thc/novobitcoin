    ...
    size_t opReturnSize=0)
{
    ...
    {
        CScript script;
        script << OP_TRUE << 0xffffffff << OP_DROP;
        tx.vout.push_back(CTxOut(Amount{10000}, script));
    }
...
BOOST_AUTO_TEST_CASE(group_recalculation_when_removing_for_block)
{
    ...
    auto entryPaysFor3 = MakeEntry(CFeeRate{Amount{2*DEFAULT_BLOCK_MIN_TX_FEE}}, {}, {std::make_tuple(entryPayForItself.GetSharedTx(), 0)}, 3);
