...
BOOST_AUTO_TEST_CASE(GetSigOpCount) {
    ...
    CScript s1;
    BOOST_CHECK_EQUAL(s1.GetSigOpCount(sigOpCountError), 0U);
    BOOST_CHECK_EQUAL(s1.GetSigOpCount(sigOpCountError), 0U);

    uint160 dummy;
    s1 << OP_1 << ToByteVector(dummy) << ToByteVector(dummy) << OP_2
       << OP_CHECKMULTISIG;
    BOOST_CHECK_EQUAL(s1.GetSigOpCount(sigOpCountError), 2U);
    s1 << OP_IF << OP_CHECKSIG << OP_ENDIF;
    BOOST_CHECK_EQUAL(s1.GetSigOpCount(sigOpCountError), 3U);
    ...
    BOOST_CHECK_EQUAL(s2.GetSigOpCount(sigOpCountError), 3U);

    CScript p2sh = GetScriptForDestination(CScriptID(s2));
    BOOST_CHECK_EQUAL(p2sh.GetSigOpCount(sigOpCountError), 0U);

    uint64_t maxPubKeysPerMultiSig = 100;   // larger than old limit
    std::vector<CPubKey> keysM;
    for (uint64_t i = 0; i < maxPubKeysPerMultiSig; i++) {
        CKey k;
        k.MakeNewKey(true);
        keysM.push_back(k.GetPubKey());
    }

    CScript s3 = GetScriptForMultisig(1, keysM);
    BOOST_CHECK_EQUAL(s3.GetSigOpCount(sigOpCountError), maxPubKeysPerMultiSig);

    // Test policy
    testConfig.Reset();
    CScript s4;
    s4 << OP_1 << ToByteVector(dummy) << maxPubKeysPerMultiSig - 1 << OP_CHECKMULTISIG;
    BOOST_CHECK_EQUAL(s4.GetSigOpCount(sigOpCountError), maxPubKeysPerMultiSig - 1);
    CScript s5;
    s5 << OP_1 << ToByteVector(dummy) << maxPubKeysPerMultiSig + 1 << OP_CHECKMULTISIG;
    BOOST_CHECK_EQUAL(s5.GetSigOpCount(sigOpCountError), maxPubKeysPerMultiSig + 1);


    CScript scriptMinus1 = CScript() << OP_1 << ToByteVector(dummy) << ToByteVector(dummy) << CScriptNum(-1) << OP_CHECKMULTISIG;
    BOOST_CHECK(scriptMinus1.GetSigOpCount(sigOpCountError) == 0); // treated as error
    BOOST_CHECK(sigOpCountError);


    CScript script_OP_9 = CScript() << OP_1 << ToByteVector(dummy) << ToByteVector(dummy) << OP_9 << OP_CHECKMULTISIG;
    BOOST_CHECK(script_OP_9.GetSigOpCount(sigOpCountError) == 9);
    BOOST_CHECK(!sigOpCountError);

    CScript script_OP_19 = CScript() << OP_1 << ToByteVector(dummy) << ToByteVector(dummy) << CScriptNum(19) << OP_CHECKMULTISIG;
    BOOST_CHECK(script_OP_19.GetSigOpCount(sigOpCountError) == 19);
    BOOST_CHECK(!sigOpCountError);
    ...
    BOOST_CHECK(script_bigNum.GetSigOpCount(sigOpCountError) == 0);
    BOOST_CHECK(sigOpCountError); // treated as error
...
void BuildTxs(CMutableTransaction &spendingTx, CCoinsViewCache &coins,
    ...
    AddCoins(coins, CTransaction(creationTx), nHeight);
}
...
BOOST_AUTO_TEST_CASE(GetTxSigOpCost) {
    ...
    // Multisig script (legacy counting)
    {
        ...
        for(int nHeight : {theHeight-1, theHeight}){
    ...
    // Multisig nested in P2SH
    {
        ...
        {
            bool sigOpCountError;
            BuildTxs(spendingTx, coins, creationTx, scriptPubKey, scriptSig, theHeight);
            BOOST_REQUIRE(GetTransactionSigOpCount(testConfig, CTransaction(spendingTx), coins,
                                                   sigOpCountError) == 0);
            BOOST_REQUIRE(VerifyWithFlag(CTransaction(creationTx), spendingTx, flags) ==
                    SCRIPT_ERR_OK);
    ...
    // Test 100 pub keys (testing policy rule)
    {
        ...
        // sig operations into account. spendingTx in itself does not contain a
        // signature operation.
        BOOST_CHECK(GetTransactionSigOpCount(testConfig, CTransaction(spendingTx), coins, sigOpCountError) == 0);
        // creationTx contains two signature operations in its scriptPubKey, but
        // legacy counting is not accurate.
        BOOST_CHECK(GetTransactionSigOpCount(testConfig, CTransaction(creationTx), coins, sigOpCountError) == 100);
        // Sanity check: script verification fails because of an invalid
        // signature.
    ...
    // Test overflow error with too big number
    {
        ...
        bool sigOpCountError;
        BuildTxs(spendingTx, coins, creationTx, scriptPubKey, scriptSig, theHeight);
        BOOST_CHECK(GetTransactionSigOpCount(testConfig, CTransaction(creationTx), coins, sigOpCountError) == 0);
...
BOOST_AUTO_TEST_CASE(test_sigops_limits) {
    ...
    std::string error;
    uint64_t expected_res = MAX_BLOCK_SIGOPS_PER_MB;

    BOOST_CHECK_EQUAL(config.GetMaxBlockSigOpsConsensus(1), expected_res);
    BOOST_CHECK_EQUAL(config.GetMaxBlockSigOpsConsensus(123456), expected_res);
    BOOST_CHECK_EQUAL(config.GetMaxBlockSigOpsConsensus(1000000), expected_res);
    BOOST_CHECK_EQUAL(config.GetMaxBlockSigOpsConsensus(1000001), 2 * expected_res);
    BOOST_CHECK_EQUAL(config.GetMaxBlockSigOpsConsensus(1348592), 2 * expected_res);
    BOOST_CHECK_EQUAL(config.GetMaxBlockSigOpsConsensus(2000000), 2 * expected_res);
    BOOST_CHECK_EQUAL(config.GetMaxBlockSigOpsConsensus(2000001), 3 * expected_res);
    BOOST_CHECK_EQUAL(config.GetMaxBlockSigOpsConsensus(2654321), 3 * expected_res);
    BOOST_CHECK_EQUAL(config.GetMaxBlockSigOpsConsensus(std::numeric_limits<uint32_t>::max()), 4295 * expected_res);
}
...
void TestMaxSigOps(const Config& globalConfig, uint64_t maxTxSigOpsCount, uint64_t maxTxSize)
{
    ...
    tx.vin[0].scriptSig = CScript() << 0xffffffff; // avoid bad-txns-undersize
    tx.vout.resize(1);
...

    {
        ...
        BOOST_CHECK(CheckRegularTransaction(CTransaction(tx), state, maxTxSize));
    }
...
BOOST_AUTO_TEST_CASE(test_max_sigops_per_tx)
{
    ...
    /* Case 1: consensus - MAX_TX_SIGOPS_COUNT */
    TestMaxSigOps(testConfig, MAX_TX_SIGOPS_COUNT, MAX_TX_SIZE_CONSENSUS);

    /* Case 2: default policy - MAX_TX_SIGOPS_COUNT_POLICY */
    uint64_t maxTxSigOpsCountPolicy = testConfig.GetMaxTxSigOpsCountPolicy();
    BOOST_CHECK_EQUAL(maxTxSigOpsCountPolicy, MAX_TX_SIGOPS_COUNT_POLICY);

    /* Case 3: policy is applied with value 0 - returns MAX_TX_SIGOPS_COUNT_POLICY */
    std::string error("");
    BOOST_CHECK(testConfig.SetMaxTxSigOpsCountPolicy(0, &error));
    BOOST_CHECK_EQUAL(error, "");
    maxTxSigOpsCountPolicy = testConfig.GetMaxTxSigOpsCountPolicy();
    BOOST_CHECK_EQUAL(maxTxSigOpsCountPolicy, MAX_TX_SIGOPS_COUNT_POLICY);

    /* Case 4: policy is applied - returns set value */
    BOOST_CHECK(testConfig.SetMaxTxSigOpsCountPolicy(3000, &error));
    BOOST_CHECK_EQUAL(error,"");
    maxTxSigOpsCountPolicy = testConfig.GetMaxTxSigOpsCountPolicy();
    BOOST_CHECK_EQUAL(maxTxSigOpsCountPolicy, 3000);

    /* Case 5: Policy is applied with too big value - previous value must not be changed */
    BOOST_CHECK(!testConfig.SetMaxTxSigOpsCountPolicy(static_cast<int64_t>(MAX_TX_SIGOPS_COUNT_POLICY) + 1, &error));
    ...
    /* Case 6: Policy is applied with negative value - previous value must not be changed */
