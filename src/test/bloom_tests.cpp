...
{
    std::vector<uint8_t> data(MAX_SCRIPT_ELEMENT_SIZE, 0xab);

    CBloomFilter filterWithShortData;
    // streamWithShortData represents filter with inserted vector std::vector<uint8_t> data(MAX_SCRIPT_ELEMENT_SIZE, 0xab)
    // Insert method does not insert data longer than MAX_SCRIPT_ELEMENT_SIZE bytes.
    ...
    // streamWithLongData represents filter with inserted vector std::vector<uint8_t> data(MAX_SCRIPT_ELEMENT_SIZE + 1, 0xab)
    ...
    data.push_back(0xab);
    // Contains method returns false if data.size() > MAX_SCRIPT_ELEMENT_SIZE.
