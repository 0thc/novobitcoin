...
static uint64_t NonPushOpCodeCount(const CScript& script)
{
    ...
    return cnt + script.GetSigOpCount(sigOpCountError);
}
                                    ...
                                    const BaseSignatureChecker& sigchecker)
{
    ...
    for (uint32_t flags : flagset) {
        ...
        if (nonPushOpcodeCount > config.GetMaxOpsPerScript(true))
        {
            BOOST_CHECK(!r.value());
        } else {
            BOOST_CHECK(r.value());
...
BOOST_AUTO_TEST_CASE(opcode_limit_tests)
{
    ...
    uint64_t testLimit = MAX_OPS_PER_SCRIPT;

    BOOST_CHECK(testLimit == GlobalConfig::GetConfig().GetMaxOpsPerScript(true));

    testLimit = MAX_TX_SIGOPS_COUNT;
    /**
