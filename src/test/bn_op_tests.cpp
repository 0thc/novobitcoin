...
BOOST_AUTO_TEST_CASE(bint_unary_ops)
{
    ...
    for(const auto& [n, arg_poly, op_code, exp_poly] : test_data)
    {
        ...
        const auto actual =
            frame.empty() ? bint{0}
                          : novo::bint::deserialize(frame.GetElement());
        const bint expected =
...
BOOST_AUTO_TEST_CASE(operands_too_large)
{
    ...
    const auto max_arg_len{ MAX_SCRIPT_NUM_LENGTH };
...
BOOST_AUTO_TEST_CASE(op_depth)
{
    ...
    for(const auto i : test_data)
    {
        ...
        vector<uint8_t> op;
        novo::serialize<int>(i, back_inserter(op));
...
BOOST_AUTO_TEST_CASE(op_size)
{
    ...
    for(const auto& [n, arg_poly] : test_data)
    {
        ...
        const auto actual{novo::deserialize(begin(stack.at(1)), end(stack.at(1)))};
        BOOST_CHECK_EQUAL(expected, actual);
    }
}
