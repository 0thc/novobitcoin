    ...
    for (size_t idx = 0; idx < tests.size(); idx++) {
        ...
        if (test[0].isArray()) {
            ...
            fValid = tx.IsCoinBase()
                                    ? CheckCoinbase(tx, state, MAX_TX_SIZE_CONSENSUS) // fixme: false genesis
                                    : CheckRegularTransaction(tx, state, MAX_TX_SIZE_CONSENSUS);

            if(!(fValid && state.IsValid())) {
                if(should_be_valid) {
                    BOOST_ERROR("Bad state of: " << strTest <<
                                "\nstate: " << state.GetRejectReason());
                } else {
...
BOOST_AUTO_TEST_CASE(basic_transaction_tests) {
    ...
    stream >> tx;
    CValidationState state;
    BOOST_CHECK_MESSAGE(CheckRegularTransaction(CTransaction(tx), state, MAX_TX_SIZE_CONSENSUS) &&
                            state.IsValid(),
...
void CreateCreditAndSpend(const CKeyStore &keystore, const CScript &outscript,
    ...
    bool ret = SignSignature(config, keystore, *output, inputm, 0,
                                   SigHashType());
...
void CheckWithFlag(const CTransactionRef &output,
    ...
    bool ret = VerifyScript(
        ...
        &error).value();
    BOOST_CHECK_MESSAGE(ret == success,
                        std::string("failed result: ") + (ret ? "true":"false"));
}
...
BOOST_AUTO_TEST_CASE(test_big_transaction) {
    ...
    for (size_t i = 0; i < mtx.vin.size(); i++) {
        bool hashSigned =
            SignSignature(testConfig, keystore, scriptPubKey, mtx, i, Amount(1000),
...
BOOST_AUTO_TEST_CASE(test_IsStandard) {
    ...
    std::string reason;
    BOOST_CHECK(IsStandardTx(testConfig, CTransaction(t), reason));

    // Check dust with default relay fee:
    Amount nDustThreshold = 3 * Amount(182 * dustRelayFee.GetFeePerK() / 1000);
    BOOST_CHECK_EQUAL(nDustThreshold, Amount(4368));
    // dust:
    ...
    // OP_FALSE OP_RETURN is standard:
    t.vout[0].scriptPubKey = CScript() << OP_FALSE << OP_RETURN;
    BOOST_CHECK(IsStandardTx(testConfig, CTransaction(t), reason));

    // OP_RETURN is not standard:
...
BOOST_AUTO_TEST_CASE(test_IsStandard_MaxTxSizePolicy)
{
    ...
    BOOST_CHECK(tx_lt_def.GetTotalSize() < config.GetMaxTxSize(false));

    // tx size greater then default max policy tx size
    int64_t size_gt_def{ static_cast<int64_t>(config.GetMaxTxSize(false) + 1) };
    AppendScriptPubKeyToFitTxSize(t, size_gt_def);
    CTransaction tx_gt_def = CTransaction(t); //
    BOOST_CHECK(tx_gt_def.GetTotalSize() > config.GetMaxTxSize(false));

    // before SetMaxTxSizePolicy
    BOOST_CHECK(IsStandardTx(config, tx_lt_def, reason));

    reason.clear();
    BOOST_CHECK(!IsStandardTx(config, tx_gt_def, reason));
...
SetupDummyInputsForConsolidationTxns(CBasicKeyStore &keystoreRet, CCoinsViewCache &coinsRet, int32_t height, bool createNonStandardInput = false) {
    ...
    AddCoins(coinsRet, CTransaction(dummyTransactions[0]), height);
    ...
    dummyTransactions[1].vout[1].scriptPubKey =
            GetScriptForDestination(key[3].GetPubKey().GetID());
    AddCoins(coinsRet, CTransaction(dummyTransactions[1]), height);
    return dummyTransactions;
}
...
std::string getVoutTypeForScriptPubKey(const CScript& scriptPubKey)
{
    CMutableTransaction t;
    t.vin.resize(1);
    t.vin[0].prevout = COutPoint(uint256(), 1);
...
BOOST_AUTO_TEST_CASE(tst_tx_toJson_OP_RETURN) {
    ...
    BOOST_CHECK_EQUAL(getVoutTypeForScriptPubKey(CScript() << OP_RETURN << ParseHex("1234")), "nonstandard");
    BOOST_CHECK_EQUAL(getVoutTypeForScriptPubKey(CScript() << OP_FALSE << OP_RETURN << ParseHex("1234")), "nulldata");
}
