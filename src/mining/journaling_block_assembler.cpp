...
{
    return ComputeMaxGeneratedBlockSize();
}
...
void JournalingBlockAssembler::threadBlockUpdate() noexcept
{
    try
    {
        LogPrint(BCLog::JOURNAL, "BlockAssembler thread starting\n");
        ...
        LogPrint(BCLog::JOURNAL, "BlockAssembler thread stopping\n");
    }
    catch(...)
    {
        LogPrint(BCLog::JOURNAL, "Unexpected exception in BlockAssembler thread\n");
    }
}
...
void JournalingBlockAssembler::updateBlock(const CBlockIndex* pindex, uint64_t maxTxns)
{
    ...
    try
    {
        ...
        if(pindex)
        {
            mLockTimeCutoff = pindex->GetMedianTimePast();
        }
        ...
        uint64_t maxBlockSizeComputed = ComputeMaxGeneratedBlockSize();
        while(!finished)
        {
    ...
    {
        LogPrint(BCLog::JOURNAL, "BlockAssembler caught: %s\n", e.what());
    }

    if(txnNum > 0)
    {
        LogPrint(BCLog::JOURNAL, "BlockAssembler processed %llu transactions from the journal\n", txnNum);
    }
}
...
{
    LogPrint(BCLog::JOURNAL, "BlockAssembler replacing journal/iterator/block\n");
...
size_t JournalingBlockAssembler::addTransaction(const CBlockIndex* pindex, uint64_t maxBlockSizeComputed)
{
    ...
    if (txn == nullptr) {
        LogPrint(BCLog::JOURNAL, "BlockAssembler found stale wrapper in the journal. need to start over.\n");
        return 0;
    }
