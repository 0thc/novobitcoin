...
static int AppInitRawTx(int argc, char *argv[]) {
    ...
    if (argc < 2 || gArgs.IsArgSet("-?") || gArgs.IsArgSet("-h") ||
        ...
        std::string strUsage =
            strprintf(_("%s novobitcoin-tx utility version"), _(PACKAGE_NAME)) +
            " " + FormatFullVersion() + "\n\n" + _("Usage:") + "\n" +
            "  novobitcoin-tx [options] <hex-tx> [commands]  " +
            _("Update hex-encoded bitcoin transaction") + "\n" +
            "  novobitcoin-tx [options] -create [commands]   " +
...
static void MutateTxAddInput(CMutableTransaction &tx,
    ...
    static const unsigned int maxVout = MAX_TX_SIZE_CONSENSUS / minTxOutSz;
...
static void MutateTxSign(const Config& config, CMutableTransaction& tx, const std::string& flagStr) {
    ...
    for (unsigned int previdx = 0; previdx < prevtxsObj.size(); previdx++) {
...

        {
            ...
            view.AddCoin(out, CoinWithScript::MakeOwning(std::move(txout), 1, false), true);
        }
