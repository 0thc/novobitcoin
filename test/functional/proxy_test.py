...
'''
Test plan:
- Start novobitcoind's with different proxy configurations
- Use addnode to initiate connections
- Verify that proxies are connected to, and the right connection command is given
- Proxy configurations to test on novobitcoind side:
...
- Create novobitcoinds that connect to them
- Manipulate the novobitcoinds using addnode (onetry) an observe effects
...
class ProxyTest(BitcoinTestFramework):
    ...
    def node_test(self, node, proxies, auth, test_onion=True):
        ...
        # Note: novobitcoind's SOCKS5 implementation only sends atyp DOMAINNAME,
        ...
        if self.have_ipv6:
            ...
            assert(isinstance(cmd, Socks5Command))
            # Note: novobitcoind's SOCKS5 implementation only sends atyp
