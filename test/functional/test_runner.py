...
def main():
    ...
    if not (enable_wallet and enable_utils and enable_bitcoind):
        print(
            "No functional tests to run. Wallet, utils, and novobitcoind must all be enabled")
        print(
...
def run_tests(test_list, build_dir, tests_dir, junitouput, exeext, tmpdir, num_jobs, enable_coverage=False, args=[], build_timings=None):
    # Warn if novobitcoind is already running (unix only)
    try:
        pidofOutput = subprocess.check_output(["pidof", "novobitcoind"])
        if pidofOutput is not None and pidofOutput != b'':
            print("%sWARNING!%s There is already a novobitcoind process running on this system. Tests may fail unexpectedly due to resource contention!" % (
                BOLD[1], BOLD[0]))
    ...
    # Set env vars
    if "NOVOBITCOIND" not in os.environ:
        os.environ["NOVOBITCOIND"] = os.path.join(
            build_dir, 'src', 'novobitcoind' + exeext)
        os.environ["NOVOBITCOINCLI"] = os.path.join(
            build_dir, 'src', 'novobitcoin-cli' + exeext)
...
def execute_test_processes(num_jobs, test_list, tests_dir, tmpdir, flags):
    ...
    # In case there is a graveyard of zombie novobitcoinds, we can apply a
...
class RPCCoverage():
    ...
    commands per `novobitcoin-cli help` (`rpc_interface.txt`).
