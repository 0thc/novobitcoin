...
"""Class for novobitcoind node under test"""
...
NOVOBITCOIND_PROC_WAIT_TIMEOUT = 60


class TestNode():
    """A class for representing a novobitcoind node under test.
    ...
    def __init__(self, i, dirname, extra_args, rpchost, timewait, binary, stderr, mocktime, coverage_dir):
        ...
        if timewait:
            ...
            self.rpc_timeout = 60
        if binary is None:
            self.binary = os.getenv("NOVOBITCOIND", "novobitcoind")
        else:
            self.binary = binary
        self.stderr = stderr
        ...
        self.cli = TestNodeCLI(
            os.getenv("BITCOINCLI", "novobitcoin-cli"), self.datadir)
    ...
    def start(self, extra_args=None, stderr=None):
        ...
        self.running = True
        self.log.debug("novobitcoind started, waiting for RPC to come up")

    def wait_for_rpc_connection(self):
        """Sets up an RPC connection to the novobitcoind process. Returns False if unable to connect."""
        # Poll at a rate of four times per second
        poll_per_s = 4
        for _ in range(poll_per_s * self.rpc_timeout):
            assert self.process.poll(
            ) is None, "novobitcoind exited with status %i during initialization" % self.process.returncode
            try:
            ...
            except ValueError as e:  # cookie file not found and no rpcuser or rpcassword. novobitcoind still starting
                if "No RPC credentials" not in str(e):
                    raise
            time.sleep(1.0 / poll_per_s)
        raise AssertionError("Unable to connect to novobitcoind")
    ...
    def wait_until_stopped(self, timeout=NOVOBITCOIND_PROC_WAIT_TIMEOUT):
        wait_until(self.is_node_stopped, timeout=timeout)

    def node_encrypt_wallet(self, passphrase):
        """"Encrypts the wallet.

        This causes novobitcoind to shutdown, so this method takes
...
class TestNodeCLI():
    """Interface to novobitcoin-cli for an individual node"""
    ...
    def __call__(self, *args, input=None):
        # TestNodeCLI is callable with novobitcoin-cli command-line args
    ...
    def send_cli(self, command, *args, **kwargs):
        """Run novobitcoin-cli command. Deserializes returned string as python object."""

        pos_args = [str(arg) for arg in args]
        named_args = [str(key) + "=" + str(value)
                      for (key, value) in kwargs.items()]
        assert not (
            pos_args and named_args), "Cannot use positional arguments and named arguments in the same novobitcoin-cli call"
