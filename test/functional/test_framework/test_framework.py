...
class BitcoinTestFramework():
    ...
    def main(self):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave novobitcoinds and test.* datadir on exit or error")
        parser.add_argument("--noshutdown", dest="noshutdown", default=False, action="store_true",
                            help="Don't stop novobitcoinds after the test execution")
        parser.add_argument("--srcdir", dest="srcdir", default=os.path.normpath(os.path.dirname(os.path.realpath(__file__)) + "/../../../src"),
                            help="Source directory containing novobitcoind/novobitcoin-cli (default: %(default)s)")
        ...
        if not self.options.noshutdown:
            ...
            self.log.info(
                "Note: novobitcoinds were not stopped and may still be running")
    ...
    def stop_node(self, i):
        """Stop a novobitcoind test node"""
        self.nodes[i].stop_node()
        self.nodes[i].wait_until_stopped()

    def stop_nodes(self):
        """Stop multiple novobitcoind test nodes"""
        for node in self.nodes:
    ...
    def assert_start_raises_init_error(self, i, extra_args=None, expected_msg=None):
        with tempfile.SpooledTemporaryFile(max_size=2**16) as log_stderr:
            try:
                ...
                assert 'novobitcoind exited' in str(e)  # node must have shutdown
                ...
                if expected_msg is None:
                    assert_msg = "novobitcoind should have exited with an error"
                else:
                    assert_msg = "novobitcoind should have exited with expected error " + expected_msg
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as novobitcoind's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        if create_cache:
            ...
            # Create cache directories, run novobitcoinds:
            for i in range(MAX_NODES):
                datadir = initialize_datadir(self.options.cachedir, i)
                args = [os.getenv("NOVOBITCOIND", "novobitcoind"), "-server",
        ...
        for i in range(self.num_nodes):
            ...
            # Overwrite port/rpcport in novo.conf
...
class ComparisonTestFramework(BitcoinTestFramework):
    ...
    Sets up some novobitcoind binaries:
    ...
    def add_options(self, parser):
        parser.add_argument("--testbinary", dest="testbinary",
                            default=os.getenv("NOVOBITCOIND", "novobitcoind"),
                            help="novobitcoind binary to test")
        parser.add_argument("--refbinary", dest="refbinary",
                            default=os.getenv("NOVOBITCOIND", "novobitcoind"),
                            help="novobitcoind binary to use for reference nodes (if any)")
