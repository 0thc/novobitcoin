UNIX BUILD NOTES
====================
Some notes on how to build Novo Bitcoin in Unix.
...
memory available when compiling Novo Bitcoin. On systems with less, gcc can be
...
See the section "Disable-wallet mode" to build Novo Bitcoin without wallet.
...
Notes
-----
The release is built with GCC and then "strip novobitcoind" to strip the debug
...
If you see memory usage blow up over time from your novobitcoind node (particularly on the STN) you
...
Note 2: When running novobitcoind, it expects boost libreries in /usr/lib64 folder. Make sure that they are located there. See libbost instalation step for details.
