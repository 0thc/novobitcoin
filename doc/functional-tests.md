...
The [/test/](/test/) directory contains integration tests that test novobitcoind
...
- [functional](/test/functional) which test the functionality of
  novobitcoind by interacting with them through the RPC and P2P
  interfaces.
- [util](/test/util) which tests the bitcoin utilities, currently only
  bitcoin-tx.

The util tests are run as part of `make check` target. Both sets of tests can also be run locally.
...
The P2P and RPC ports used by the novobitcoind nodes-under-test are chosen to make
conflicts with other processes unlikely. However, if there is another novobitcoind
process running on the system (perhaps from a previous test which hasn't successfully
killed all its novobitcoind nodes), then there may be a port conflict which will
cause the test to fail. It is recommended that you run the tests on a system
where no other novobitcoind processes are running.

On linux, the test_framework will warn if there is another
novobitcoind process running when the tests are started.

If there are zombie novobitcoind processes after test failure, you can kill them
by running the following commands. **Note that these commands will kill all
novobitcoind processes running on the system, so should not be used if any non-test
novobitcoind processes are being run.**

```bash
killall novobitcoind
```

or

```bash
pkill -9 novobitcoind
```
...
sure novobitcoind processes are stopped as above):
...
- When run on Travis, no logs are output to the console. However, if a test
  fails, the `test_framework.log` and novobitcoind `debug.log`s will all be dumped
  to the console to help troubleshooting.

To change the level of logs output to the console, use the `-l` command line
argument.

`test_framework.log` and novobitcoind `debug.log`s can be combined into a single
...
call methods that interact with the novobitcoind nodes-under-test.

If further introspection of the novobitcoind instances themselves becomes
...
```bash
cat /tmp/user/1000/testo9vsdjo3/node1/regtest/novo.pid
gdb /home/example/novobitcoind <pid>
```
...
- P2P tests have two threads. One thread handles all network communication
  with the novobitcoind(s) being tested in a callback-based event loop; the other
  implements the test logic.
- `P2PConnection` is the class used to connect to a novobitcoind.  `P2PInterface`
  contains the higher level logic for processing P2P payloads and connecting to
  the Novo Bitcoin node application logic. For custom behaviour, subclass the
...
Basic code to support P2P connectivity to a novobitcoind.
